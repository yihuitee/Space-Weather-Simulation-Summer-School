...
def implicit_euler_step(f,x,t,h):    
    #update function
    update = lambda x_hat : h*f(x_hat,t+h) + x - x_hat
    #solve via fsolve
    x_hat = fsolve(update, x)
    return x_hat
    #return ... # please complete this function
...
def crank_nicolson_step(f,x,t,h):
    ...
    y = x + h/2*f(x,t)
    # implicit update
    update = lambda x_hat : h/2*f(x_hat, t+h) + y - x_hat
    # solve imlpicit update
    x_hat = fsolve(update, y)
    return x_hat
    #return ... # please complete this function
...
def heun_step(f,x,t,h):
    ...
    x_ee = x+h*k_1
    k_2 = f(x_ee, t+h)
    return x + h/2 * (k_1+k_2) # please complete this function
...
colors = {explicit_euler_step : "red",
          implicit_euler_step : "dodgerblue",
          heun_step : "green",
          crank_nicolson_step : "black"}
